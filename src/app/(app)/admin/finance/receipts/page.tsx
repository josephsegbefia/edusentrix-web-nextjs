...
export default async function AdminFinanceReceiptsPage() {
  const ctx = await requireSchoolAdmin();
  await connectToDatabase();

  const [feePayments, learnPayments] = await Promise.all([
    Payment.find({ schoolId: ctx.schoolId, status: "completed" })
      .sort({ paymentDate: -1, createdAt: -1 })
      .limit(150)
      .select("_id studentId amountMinor status paymentDate createdAt paystackReference externalReference receiptNumber")
      .lean(),
    LearnPaymentIntent.find({ schoolId: ctx.schoolId })
      .sort({ createdAt: -1 })
      .limit(150)
  ...
  const studentIds = Array.from(
    new Set([...feePayments, ...learnPayments].map((row) => String(row.studentId)).filter(Boolean)),
  );
  ...
  const rows: ReceiptRow[] = [
    ...feePayments.map((payment) => ({
      ...
      studentName: studentName(studentMap.get(String(payment.studentId))),
    ...
    <div className="p-6 text-white md:p-8">
      <WorkspacePageShell>
        <WorkspacePageHeader
          ...
          subtitle="School fee and EduSentrix Learn receipts visible to this school."
          iconName="file-text"
        ...
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <Metric label="Tracked records" value={rows.length.toLocaleString()} />
        </div>
