// src/components/parent/academics/SubjectPerformanceTable.tsx
"use client";
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-white/10 bg-slate-950/80">
      <div className="grid grid-cols-12 border-b border-white/10 bg-white/5 px-4 py-2 text-[11px] font-medium uppercase tracking-wide text-slate-300">
        ...
        <div className="col-span-2 text-right hidden md:block">CA</div>
        <div className="col-span-2 text-right hidden md:block">Exam</div>
        <div className="col-span-4 md:col-span-2 text-right">Total</div>
        <div className="col-span-2 md:col-span-1 text-center">Grade</div>
        <div className="col-span-2 text-right hidden lg:block">Teacher</div>
        {onViewBreakdown && (
          <div className="col-span-1 text-center hidden md:block">Details</div>
        )}
      </div>
      <div className="divide-y divide-white/5">
        {subjects.map((s) => (
          <div
            key={s.subjectId}
            className="grid grid-cols-12 items-center px-4 py-3 text-xs text-slate-100/90 hover:bg-white/5"
          >
            <div className="col-span-4 md:col-span-3 flex flex-col">
              ...
              {/* Show teacher on mobile */}
              <span className="text-[10px] text-slate-400 lg:hidden">
                {s.teacherName ?? ""}
              </span>
            </div>
            <div className="col-span-2 text-right text-[11px] hidden md:block">
            ...
            <div className="col-span-4 md:col-span-2 text-right text-[11px] font-semibold">
            ...
            <div className="col-span-2 text-right text-[11px] text-slate-300 hidden lg:block">
              {s.teacherName ?? "--"}
            </div>
            {onViewBreakdown && (
