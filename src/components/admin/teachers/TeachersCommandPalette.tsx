...
import type { TeachersTabId } from "@/constants/teachers";

type TeachersCommandPaletteProps = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onFocusSearch: () => void;
  onGoToTab: (tab: TeachersTabId) => void;
  onCreateTeacher: () => void;
  onImportTeachers: () => void;
};

export function TeachersCommandPalette({
  open,
  onOpenChange,
  onFocusSearch,
  onGoToTab,
  ...
  const items = React.useMemo(
    () => [
      ...
      {
        id: "new-teacher",
        label: "Add a new teacher",
        hint: "N",
        icon: <Plus className="h-4 w-4" />,
        action: onCreateTeacher,
      },
      {
        id: "import-teachers",
        label: "Import teachers from CSV",
        icon: <Upload className="h-4 w-4" />,
        action: onImportTeachers,
      },
      {
        id: "tab-all",
      ...
      {
        id: "tab-active",
        label: "View active teachers",
        icon: <Users className="h-4 w-4" />,
        action: () => onGoToTab("active"),
      },
      {
        id: "tab-homeroom",
        label: "View homeroom teachers",
        icon: <Users className="h-4 w-4" />,
        action: () => onGoToTab("homeroom"),
      },
    ],
    [onFocusSearch, onCreateTeacher, onImportTeachers, onGoToTab]
  );
