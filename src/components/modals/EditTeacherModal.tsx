...
import { UpdateTeacherSchema, type UpdateTeacherInput } from "@/schemas/teacher";
...
const STEPS = [
  { id: 1, title: "Personal Details", fields: ["firstName", "lastName", "email", "phone"] },
  { id: 2, title: "Photo & Status", fields: ["photoUrl", "status"] },
  { id: 3, title: "Professional Info", fields: ["employeeId", "department", "hireDate", "terminationDate", "maxClasses", "maxStudents"] },
  { id: 4, title: "Assignments", fields: ["subjectIds", "homeroomClassGroupId", "notes"] },
] as const;
...
export default function EditTeacherModal({
  ...
  const {
    ...
    defaultValues: {
      ...
      terminationDate: teacher.terminationDate ? teacher.terminationDate.split("T")[0] : "",
    ...
    (async () => {
      ...
      try {
        const res = await fetch(
          `/api/admin/class-groups/search?q=${encodeURIComponent(dqClass)}&limit=12`,
          { cache: "no-store" }
        );
  ...
  async function internalSubmit(values: UpdateTeacherInput) {
    ...
    if (values.firstName !== teacher.firstName) payload.firstName = values.firstName;
    if (values.lastName !== teacher.lastName) payload.lastName = values.lastName;
    if (values.email !== (teacher.email ?? "")) payload.email = values.email;
    if (values.phone !== (teacher.phone ?? "")) payload.phone = values.phone || null;
    if (values.photoUrl !== (teacher.photoUrl ?? undefined)) payload.photoUrl = values.photoUrl || null;
    if (values.status !== teacher.status) payload.status = values.status;
    if (values.employeeId !== (teacher.employeeId ?? "")) payload.employeeId = values.employeeId || null;
    if (values.department !== (teacher.department ?? "")) payload.department = values.department || null;

    const originalHireDate = teacher.hireDate ? teacher.hireDate.split("T")[0] : "";
    const originalTermDate = teacher.terminationDate ? teacher.terminationDate.split("T")[0] : "";
    if (values.hireDate !== originalHireDate) payload.hireDate = values.hireDate || null;
    if (values.terminationDate !== originalTermDate) payload.terminationDate = values.terminationDate || null;

    if (values.maxClasses !== (teacher.maxClasses ?? undefined)) payload.maxClasses = values.maxClasses;
    if (values.maxStudents !== (teacher.maxStudents ?? undefined)) payload.maxStudents = values.maxStudents;
    if (values.notes !== (teacher.notes ?? "")) payload.notes = values.notes || null;

    const originalSubjectIds = teacher.subjects.map((s) => s.id).sort().join(",");
            ...
            <section className="space-y-4">
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div className="space-y-2">
                  ...
                  />
                  {errors.firstName && (
                    <div className="text-xs text-rose-300">{errors.firstName.message}</div>
                  )}
                </div>
                <div className="space-y-2">
                  <Label
                  ...
                  />
                  {errors.lastName && (
                    <div className="text-xs text-rose-300">{errors.lastName.message}</div>
                  )}
                </div>
              </div>
          ...
          {currentStep === 2 && (
            <section className="space-y-6">
              <div className="space-y-4">
                ...
                {errors.photoUrl && (
                  <div className="text-xs text-rose-300 text-center">{errors.photoUrl.message}</div>
                )}
              </div>
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <div className="grid grid-cols-2 gap-3">
                      {(["active", "inactive", "on_leave", "terminated"] as TeacherStatus[]).map((s) => (
                        <motion.label
                          key={s}
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    type="number"
                    min={0}
                    {...register("maxClasses", { valueAsNumber: true })}
                    placeholder="e.g., 6"
                    className="border border-white/10 bg-white/5 text-white placeholder:text-muted focus:border-brand focus:ring-1 focus:ring-brand"
                  />
                </div>
            ...
            <section className="space-y-6">
              <div className="space-y-4">
                ...
                <div className="max-h-64 overflow-y-auto rounded-lg border border-white/10 bg-white/5 p-4">
                  {subjectsLoading ? (
                    <div className="text-xs text-white/50 py-4 text-center">Loading subjects…</div>
                  ) : displaySubjects.length === 0 ? (
                    <div className="text-xs text-white/50 py-4 text-center">
                      {qSubj.trim() ? "No subjects found matching your search" : "No subjects assigned. Search to add subjects."}
                    </div>
                  ) : (
                      ...
                      {displaySubjects.map((s) => {
                        ...
                        return (
                          <motion.button
                            ...
                            whileHover={{ scale: 1.02 }}
                            whileTap={{ scale: 0.98 }}
                            onClick={() => toggleSubject(s._id, s.name)}
                ...
                {subjectIds.length > 0 && (
                  <p className="text-xs text-white/50">
                    {subjectIds.length} subject{subjectIds.length !== 1 ? "s" : ""} assigned
                  </p>
                )}
              </div>
                    ...
                    <div className="max-h-64 overflow-y-auto rounded-lg border border-white/10 bg-white/5 p-4">
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                        ...
                        >
                          ...
                          <span className="font-semibold text-sm">No homeroom</span>
                        </motion.button>

                        {/* Current homeroom if exists */}
                        {teacher.homeroom && !classResults.find((c) => c._id === teacher.homeroom?.id) && (
                          <motion.button
                            type="button"
                            whileHover={{ scale: 1.02 }}
                            whileTap={{ scale: 0.98 }}
                            onClick={() => field.onChange(teacher.homeroom?.id)}
                            className={`relative rounded-lg border-2 px-4 py-3 text-left transition-all ${
                          ...
                          >
                            ...
                            <span className="font-semibold text-sm">{teacher.homeroom.name}</span>
                          </motion.button>
                        )}
                            ...
                            return (
                              ...
                              >
                                ...
                                <div className="flex flex-col">
                                  <span className="font-semibold text-sm">{cg.name}</span>
                                  {cg.gradeLabel && (
                                    <span className="text-xs opacity-60">{cg.gradeLabel}</span>
                                  )}
                                </div>
                              </motion.button>
                            );
