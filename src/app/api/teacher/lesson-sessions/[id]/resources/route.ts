...
async function assertSessionResourceAccess(
  ...
  if (loaded.kind === "not_found") return { ok: false as const, status: 404 };
  if (loaded.kind === "forbidden") return { ok: false as const, status: 403 };
  if (
  ...
  return { ok: true as const, session: loaded.session };
}
...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const access = await assertSessionResourceAccess(
      sessionId,
      context.schoolId,
      context.teacherId,
      context.isAdmin,
    ...
    if (!access.ok) {
      return Response.json(
        { success: false, error: access.status === 403 ? "Forbidden" : "Session not found" },
  ...
  try {
    ...
    if (parsed.data.kind === "file") {
      ...
      try {
        ...
        if (!deleted) {
          console.error("Rollback failed for lesson session resource upload:", fileUrl);
        }
        throw error;
      }
    }
