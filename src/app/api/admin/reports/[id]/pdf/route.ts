// src/app/api/admin/reports/[id]/pdf/route.ts
...
import type { PeriodReportType } from "@/models/PeriodReport";
...
function toVerificationReportType(
  reportType: PeriodReportType
): "term_report" | "weekly_report" | "monthly_report" | "biweekly_report" {
  switch (reportType) {
    case "term":
      return "term_report";
    case "weekly":
...
export async function GET(
  ...
  try {
    ...
    const reportIdObj = new mongoose.Types.ObjectId(id);
    const userIdObj =
    ...
    const reportTypeLabel =
      report.reportType === "term"
        ? "Academic Term Report"
        : report.reportType === "weekly"
          ? "Weekly Report"
          : report.reportType === "biweekly"
            ? "Biweekly Report"
            : "Monthly Report";

    const reportLabel = `${reportTypeLabel} - ${periodLabel}`;
    ...
    const verificationReportType = toVerificationReportType(report.reportType);
    const verificationPayload = {
      ...
      meta: {
        categories: ["academic", report.reportType, "report"],
        version: 1,
      },
      issuedAt: new Date(),
    };
    ...
    await recordActivity({
      ...
      entityId: reportIdObj,
      description: `Generated ${report.reportType} report PDF`,
      metadata: {
        reportType: verificationReportType,
        verificationId,
        reportId: id,
    ...
    const fileName = `${report.reportType}-report-${periodLabel.replace(/\s+/g, "-")}-${verificationId.slice(-8)}.pdf`;
