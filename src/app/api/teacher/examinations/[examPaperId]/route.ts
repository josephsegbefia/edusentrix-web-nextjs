...
import { teacherCanEditExamPaper, teacherCanViewExamPaper } from "@/lib/examinations/paper-access";
import {
...
export async function PATCH(
  ...
  try {
    ...
    if (!teacherCanEditExamPaper(existing, ctx)) {
      return Response.json(
        { success: false, error: "Only draft or revision-requested papers assigned to you can be edited" },
        { status: 403 }
      );
    }
...
export async function DELETE(
  ...
  try {
    const ctx = await requireTeacher();
    if (!can(ctx.permissions, PERMISSIONS.examsDelete)) {
      return Response.json({ success: false, error: "Forbidden" }, { status: 403 });
    }
    await connectToDatabase();
    const { examPaperId } = await params;
    ...
    if (!existing) return Response.json({ success: false, error: "Exam paper not found" }, { status: 404 });
    if (!teacherCanEditExamPaper(existing, ctx)) {
      return Response.json({ success: false, error: "Only editable papers can be deleted" }, { status: 403 });
    }
