...
export async function GET(
  ...
  try {
    ...
    // Session must be published to students, or have a completed delivery for this class
    ...
    if (session.studentVisibility !== "published") {
      ...
      if (!completedDelivery) {
        return Response.json({ success: false, error: "Session not available" }, { status: 403 });
      }
    }
    ...
    const uniqueBookIds = [...new Set(libraryIds.map(String))].map(
    ...
    console.error("[student/lesson-sessions/[id]/resources GET]", e);
