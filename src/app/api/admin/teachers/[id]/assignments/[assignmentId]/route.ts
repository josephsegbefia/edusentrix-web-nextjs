...
export async function PATCH(
  ...
  if (payload.subjectId !== undefined) {
    ...
    const subject = await Subject.findOne({ _id: subjectObjId, schoolId: schoolIdObj, isActive: true });
    if (!subject) {
      return Response.json({ error: "Subject not found or inactive" }, { status: 400 });
    }
    if (existingAssignment.subjectId.toString() !== payload.subjectId) {
      changes.push({ field: "subjectId", from: existingAssignment.subjectId.toString(), to: payload.subjectId });
  ...
  if (payload.classGroupId !== undefined) {
    ...
    if (!classGroupObjId) {
      return Response.json({ error: "Invalid class group ID" }, { status: 400 });
    }
    const classGroup = await ClassGroup.findOne({ _id: classGroupObjId, schoolId: schoolIdObj });
    if (!classGroup) {
    ...
    if (existingAssignment.classGroupId.toString() !== payload.classGroupId) {
      changes.push({ field: "classGroupId", from: existingAssignment.classGroupId.toString(), to: payload.classGroupId });
  ...
  if (payload.academicPeriodId !== undefined) {
    ...
    const period = await AcademicPeriod.findOne({ _id: periodObjId, schoolId: schoolIdObj });
    if (!period) {
      return Response.json({ error: "Academic period not found" }, { status: 400 });
    }
    if (existingAssignment.academicPeriodId.toString() !== payload.academicPeriodId) {
      changes.push({ field: "academicPeriodId", from: existingAssignment.academicPeriodId.toString(), to: payload.academicPeriodId });
      updateFields.academicPeriodId = periodObjId;
    }
  }

  // Update status
  if (payload.status !== undefined && payload.status !== existingAssignment.status) {
    changes.push({ field: "status", from: existingAssignment.status, to: payload.status });
    updateFields.status = payload.status;
  }

  // Update workloadHours
  if (payload.workloadHours !== undefined && payload.workloadHours !== existingAssignment.workloadHours) {
    changes.push({ field: "workloadHours", from: existingAssignment.workloadHours, to: payload.workloadHours });
  ...
  if (payload.notes !== undefined) {
    ...
    if (newNotes !== existingAssignment.notes) {
      changes.push({ field: "notes", from: existingAssignment.notes, to: newNotes });
  ...
  if (payload.schedules !== undefined) {
    if (payload.schedules === null) {
      ...
      changes.push({ field: "schedules", from: existingAssignment.schedules?.length || 0, to: payload.schedules.length });
    }
  }

  // No changes
  if (Object.keys(updateFields).length === 0) {
    return Response.json({ success: true, data: { id: assignmentId }, warnings: ["No changes detected"] });
  }
  ...
  if (finalStatus === "active") {
    const finalSubjectId = updateFields.subjectId ?? existingAssignment.subjectId;
    const finalClassGroupId = updateFields.classGroupId ?? existingAssignment.classGroupId;
    const finalPeriodId = updateFields.academicPeriodId ?? existingAssignment.academicPeriodId;
    ...
    if (conflictingAssignment) {
      const subject = conflictingAssignment.subjectId as unknown as { name?: string };
      const classGroup = conflictingAssignment.classGroupId as unknown as { name?: string };
      return Response.json(
        {
