...
export function PublicStudentParentDocumentUploadView({ token }: { token: string }) {
  const [meta, setMeta] = React.useState<ParentDocMeta | null>(null);
  ...
  React.useEffect(() => {
    ...
    (async () => {
      ...
      setError(null);
      try {
        const res = await fetch(`/api/public/students/parent-documents/${token}`, {
          cache: "no-store",
        });
        ...
        setMeta(json.data as ParentDocMeta);
      } catch (e) {
        if (!cancelled) {
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1 text-xs text-white/50">
                Student:{" "}
                <span className="font-medium text-white/85">{meta.studentName}</span>
                {meta.admissionNo ? (
                  <>
                    {" "}
                    · ID{" "}
                    <span className="font-mono text-white/80">{meta.admissionNo}</span>
                  </>
                ) : null}
              </p>
            </div>
        ...
        {doc ? (
          <div className="flex items-start gap-3 rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-4 text-sm">
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-white/60">
                Thank you. The school has received your file. You can close this
                page.
              </p>
            </div>
          </div>
          ...
          <>
            ...
            <PublicDocumentUploader
              ...
              studentParentDocumentToken={token}
              value={doc}
              onChange={setDoc}
            />
          </>
