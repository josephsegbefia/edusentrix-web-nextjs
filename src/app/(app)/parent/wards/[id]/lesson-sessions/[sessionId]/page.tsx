...
export default function ParentWardLessonSessionPage() {
  const params = useParams();
  const wardId = typeof params.id === "string" ? params.id : null;
  const sessionId = typeof params.sessionId === "string" ? params.sessionId : null;

  const { data, isLoading, isError, error } = useQuery({
    queryKey: ["parent-ward-lesson-session-detail", wardId, sessionId],
    queryFn: async () => {
      const res = await fetch(
        `/api/parent/wards/${wardId}/lesson-sessions/${sessionId}`,
        { cache: "no-store" }
      );
    ...
    enabled: Boolean(wardId && sessionId),
  });
