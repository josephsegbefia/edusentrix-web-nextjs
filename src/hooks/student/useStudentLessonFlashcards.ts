...
export function useStudentLessonFlashcards(lessonId: string | null, enabled = true) {
  return useQuery<StudentLessonFlashcardsResponse>({
    queryKey: ["student-lesson-flashcards", lessonId],
    queryFn: async () => {
      const res = await fetch(`/api/student/lessons/${lessonId}/flashcards`, {
        cache: "no-store",
      });
...
export function useStudentFlashcardProgress(lessonId: string | null) {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: ["student-lesson-flashcards", lessonId] });
    },
  });
}
