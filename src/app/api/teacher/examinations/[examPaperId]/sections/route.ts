...
import { createSectionAction } from "@/lib/examinations/builder-route-actions";
...
export async function POST(
  ...
  try {
    ...
    const result = await createSectionAction(req, {
    ...
    return Response.json(
      { success: false, error: error instanceof Error ? error.message : "Failed to create section" },
      { status: 500 }
    );
  }
}
