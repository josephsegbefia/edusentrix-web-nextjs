...
export async function POST(
  ...
  try {
    ...
    const result = await returnAdminReportRun(
    ...
    console.error("Admin report run return POST:", error);
    return NextResponse.json(
      { success: false, error: "Failed to return report run" },
      { status: 500 }
    );
  }
}
