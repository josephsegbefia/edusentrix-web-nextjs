...
export async function POST(
  ...
  try {
    ...
    const data = await draftExamParentMessageWithLeo({
    ...
    console.error("Exam Leo draft-parent-message POST:", error);
    return NextResponse.json(
      { success: false, error: "Failed to draft parent message" },
      { status: 500 }
    );
  }
}
