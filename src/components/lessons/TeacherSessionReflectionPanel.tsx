...
import {
  useTeacherSessionReflection,
  useTeacherUpsertSessionReflection,
} from "@/hooks/teacher/useTeacherSessionReflection";
...
export function TeacherSessionReflectionPanel({ sessionId, canWrite }: Props) {
  const busyToast = useBusyToast();
  const { data, isLoading, error } = useTeacherSessionReflection(sessionId, true);
  const saveMut = useTeacherUpsertSessionReflection(sessionId);
    ...
    >
      <CardHeader className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <CardTitle className="text-lg text-white">Delivery reflection</CardTitle>
            <p className="text-xs text-white/50">Tied to this class delivery — outcomes and follow-up</p>
          </div>
        </div>
        {canWrite && (
          <Button
          ...
          <>
            <div className="flex flex-wrap items-center justify-between gap-4 rounded-xl border border-white/10 bg-white/5 px-4 py-3">
              <div className="space-y-1">
                ...
                <p className="text-xs text-white/45">Use when you’ve delivered this session.</p>
              </div>
              <Switch
                checked={completed}
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                placeholder="Homework, reteach, or link to the next session"
              />
            </div>
          </>
        )}
